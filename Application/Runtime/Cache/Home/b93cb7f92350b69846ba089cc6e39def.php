        ...
        <div class="mt20">
        	<div class="p10">
                ...
                <ul class="ch_ul quesTypes" >
                	<?php if(is_array($tiku_type)): $i = 0; $__LIST__ = $tiku_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
						<div><?php echo ($vo["type_name"]); ?></div>
						<div style="height:30px;margin-left:15px;" id="<?php echo ($vo["id"]); ?>">
							...
							<div style="float:left;">	
								<input value="0"  maxlength="3"  class="countInput" name="tx[<?php echo ($vo["id"]); ?>]" type="text">
							</div>
							<div style="padding:4px 0;float:left;">	
		...
		<div class="mt10">
			...
			<div style="height: auto; padding: 0px;" class="knowledgeTxtBox">
				<div class="knowledgeTxt guideStep">
					<h3><font>您已经选择了<b class="zs_num">0</b>个知识点。</font><a style="display: block;" href="javascript:;" class="btn btn9" id="all_">选择全部知识点</a><a href="javascript:;" class="btn btn10" id="no_all_" style="display: none;">取消全部知识点</a></h3>
					...
					<form method="post" action="/smart/start" id="start">
					...
					<div class="guideStep">
						...
						<div class="clearBoth fl mb">
							<?php if(is_array($tiku_type)): $i = 0; $__LIST__ = $tiku_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input type="hidden" name="tixing[<?php echo ($vo["id"]); ?>]" value="" class="tixing" id="tixing_<?php echo ($vo["id"]); ?>" /><?php endforeach; endif; else: echo "" ;endif; ?>
							<input type="hidden" name="point_num" value="0" id="point_num" />
							<input type="hidden" id="difficulty_id" name="difficulty_id" value="1" />
							<a  id="submit_ajax" class="btn btn5" style="margin-left:270px;" >生成试卷</a>
						</div>
...
<script>
	...
	$('.add').click(function(){
		...
		obj.val(num);
		$('#tixing_'+$(this).parent().parent().attr('id')).val(num);
	});
	...
	$('.countInput').blur(function() {
		if(parseInt($(this).val())=='NaN'){
			...
			$('#tixing_'+$(this).parent().parent().attr('id')).val(parseInt($(this).val()));
		}
	});
	$('#submit_ajax').click(function(){
		var rs = 0;
		$('.tixing').each(function(dom){
			if(parseInt($(this).val())>0) rs=1;
		});

...
<body>
<div class="pd-20">
	<form action="<?php echo U('/admin/admin/roleedit');?>" method="post" class="form form-horizontal" id="form-user-character-add">
		<input type="hidden" name="id" value="<?php echo ($group["id"]); ?>" />
		<div class="row cl">
			<label class="form-label col-2"><span class="c-red">*</span>角色名称：</label>
			<div class="formControls col-10">
				<input type="text" disabled="" class="input-text" value="<?php echo ($group["title"]); ?>" placeholder="" id="user-name" name="title" datatype="*4-16" nullmsg="角色名称不能为空">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-2">描述：</label>
			<div class="formControls col-10">
				<input type="text" class="input-text" value="<?php echo ($group["description"]); ?>" placeholder="" id="" name="description">
			</div>
		</div>
		<div class="row cl">
						...
						<dl class="cl permission-list2">
							...
							<dd>
								<?php if(is_array($vo["childs"])): $i = 0; $__LIST__ = $vo["childs"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><label class="">
									<input type="checkbox" value="<?php echo ($v["id"]); ?>" <?php if(in_array($v['id'],$group['rules'])){ ?>checked=""<?php } ?> name="rules[]" id="user-Character-0-0-0">
